    ...
    public class ReviewController : Controller
    {
        ...
        public IActionResult GetReviewOfPokemon(int pokeId)
        {
            if (!_reviewRepository.ReviewExists(pokeId))
                return NotFound();
            var review = _mapper.Map<PokemonDto>(_reviewRepository.GetReviewsOfAPokemon(pokeId));
        ...
        [ProducesResponseType(204)]
        [ProducesResponseType(404)]
        public IActionResult DeleteReviewsByReviewer(int reviewId)
        {
            if (!_reviewRepository.ReviewExists(reviewId))
                return NotFound();

            var reviewToDelete = _reviewerRepository.GetReviewByReviewer(reviewId).ToList();
